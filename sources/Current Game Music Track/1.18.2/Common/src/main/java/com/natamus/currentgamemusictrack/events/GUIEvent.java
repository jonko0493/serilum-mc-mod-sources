...
public class GUIEvent {
	public static void renderOverlay(PoseStack poseStack, float tickDelta) {
		...
		drawText(font, poseStack, musicGUIString, titleXCoord, height - ConfigHandler.musicGUIBottomHeightOffset, colour, ConfigHandler.drawTextShadow);
	...
	private static void drawText(Font font, PoseStack poseStack, String content, int x, int y, Color colour, boolean drawShadow) {
		if (Variables.guiOpacity > 10 && !content.equals("")) {
			if (drawShadow) {
				font.drawShadow(poseStack, content, x, y, colour.getRGB());
				return;
			}
			font.draw(poseStack, content, x, y, colour.getRGB());
		}
	}
}